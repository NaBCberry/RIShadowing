...
def pyinstaller_build():
    ...
    exe_dir = DIST_WORK / "AI_Interpreter"
...
def make_installer(version: str, include_model: bool):
    ...
    if include_model:
        iss_content = iss_content.replace(
            "; Source: \"dist_work\\AI_Interpreter\\vosk-model*", "Source: \"dist_work\\AI_Interpreter\\vosk-model*"
        )
    ...
    installer_name = f"AI_Interpreter_{version}_{suffix}_Setup.exe"
...
def make_portable(version: str, include_model: bool):
    ...
    zip_name = f"AI_Interpreter_{version}_Portable_{label}.zip"
...
def main():
    parser = argparse.ArgumentParser(
        description="AI_Interpreter Build Script",
    )
