...
| 文件 | 职责 |
|------|------|
| `src/app.py` | **`ShadowingApp` 类**（~915 行）。构建 GUI、协调录音/播放/TTS/ASR/比较全过程；维护状态机（generate → shadowing → finished）；每 100ms 轮询更新 UI（含电平表、光标、评分）；训练屏左侧含竖向录音电平表 |
...
│      │  TRAINING SCREEN（训练屏）         │
│ 电   │  ┌─────────────────────────────┐│
│ 平   │  │ Feedback Panel (Speed+Acc)  ││
│ 表   │  │ Display Panel (Ref+User)    ││
│      │  ├─────────────────────────────┤│
│      │  │ [TERMINATE]                 ││
│      │  └─────────────────────────────┘│
│      │  RUNNING: 播放+录音+实时STT      │
